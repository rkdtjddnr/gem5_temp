...
CPU_CONFIG="--param=system.cpu[0:4].l2cache.mshrs=46 --param=system.cpu[0:4].dcache.mshrs=20 \
  ...
  --param=system.iobus.is_ioxbar=True --param=system.iobus.frontend_latency=20 --param=system.iobus.forward_latency=15 --param=system.iobus.response_latency=35"
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/dpdk-set-1core-l3-2port-4ns-head-35ns/$num_nics"NIC-"$num_queues"Qs-"$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$RATE"Gbps-ddio-enabled"-$GUEST_SCRIPT
  setup_dirs
