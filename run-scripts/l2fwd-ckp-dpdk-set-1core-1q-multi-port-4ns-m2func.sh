...
function run_simulation {
  ...
  --num-nics="$num_nics" --num-loadgens="$num_nics" --num-queues="$num_queues" --is-m2func \
...
RESOURCES=${GIT_ROOT}/resources-dpdk-m2func-wo-printf
...
CKPT_DIR=${GIT_ROOT}/ckpts/"m2func-"$num_nics"NIC"-$num_queues"Qs"-$GUEST_SCRIPT-"wo-printf"
if [[ -z "$num_nics" ]]; then
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/m2func-SingleQueue-wo-printf/$num_nics"NIC-"$num_queues"Qs-1core-ckp-"$GUEST_SCRIPT
  setup_dirs
  ...
  RUNDIR=${GIT_ROOT}/rundir/m2func-dpdk-set-1core-l3-2port-4ns-wo-printf-rd-distance/$num_nics"NIC-"$num_queues"Qs-"$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$RATE"Gbps-ddio-enabled"-$GUEST_SCRIPT
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  # DEBUG_FLAGS="--debug-flags=EthernetDpdk,LoadgenDebug"
