...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/ISPASS-2024-touchfwd-enso/$num_nics"NIC-"$num_queues"Queues-"$GUEST_SCRIPT
  setup_dirs
  ...
  RUNDIR=${GIT_ROOT}/rundir/dpdk-testpmd-touchfwd/$num_nics"NIC-"$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$RATE"Gbps-ddio-enabled"-$GUEST_SCRIPT
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG  --cpu-clock=$Freq -r 3 --loadgen-start=8268734826462 --rel-max-tick=400300000000 --packet-rate=$PACKET_RATE --packet-size=$PACKET_SIZE --loadgen-mode=$LOADGENMODE \
  --warmup-dpdk 20000000"
