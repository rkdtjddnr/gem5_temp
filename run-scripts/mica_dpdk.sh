...
TEMP=$(getopt -o 'h' --long freq:,take-checkpoint,num-nics:,cpu-types:,l2-size:,script:,packet-rate:,num-queues:,pcap-file:,loadgen-find-bw,help -n 'dpdk-loadgen' -- "$@")
...
if [[ -n "$checkpoint" ]]; then
  ...
  if [[ -z "$PCAP_FILENAME" ]]; then
    echo "Error: missing argument --pcap_file" >&2
    usage
  fi
  PORT=11211
  #PCAP_FILENAME="../resources-dpdk/request-dpdk-10k.pcap"
  # PCAP_FILENAME="../resources/request-dpdk-trace.pcap"
  #PCAP_FILENAME="../resources-dpdk/replay_trace/mica_16k/mica_4k_0.pcap"
  ...
  --loadgen_pcap_filename=$PCAP_FILENAME --loadgen-start=7766448493717 --packet-rate=$PACKET_RATE \
