...
CACHE_CONFIG="--caches --l2cache --l3cache --l3_multiport --l3_size 16MB --l3_assoc 32 --ddio-enabled --l1i_size=512kB --l1i_assoc=16 \
--l1d_size=512kB --l1d_assoc=16 --l2_size=2MB --l2_assoc=16 --cacheline_size=64 --l3_cpu_side_ports_connection_count 2" 
CPU_CONFIG="--param=system.l3.mshrs=256 --param=system.cpu[0:4].l2cache.mshrs=144 --param=system.cpu[0:4].dcache.mshrs=144 --param=system.cpu[0:4].icache.mshrs=80 \
  ...
  --param=system.cpu[0:4].l2cache.data_latency=8 --param=system.cpu[0:4].l2cache.response_latency=8 --param=system.cpu[0:4].l2cache.tag_latency=8 \
  ...
  --param=system.switch_cpus[0:4].numROBEntries=1024 --param=system.switch_cpus[0:4].numIQEntries=1024 \
  --param=system.switch_cpus[0:4].LQEntries=512 --param=system.switch_cpus[0:4].SQEntries=512 \
  --param=system.switch_cpus[0:4].numPhysIntRegs=512 --param=system.switch_cpus[0:4].numPhysFloatRegs=512 --param=system.switch_cpus[0:4].numPhysVecRegs=512 --param=system.switch_cpus[0:4].numPhysVecPredRegs=64 \
  --param=system.switch_cpus[0:4].branchPred.BTBEntries=65536 --param=system.switch_cpus[0:4].issueWidth=8 \
  ...
  --param=system.switch_cpus[0:4].fuPool.FUList[0].count=64 \
  --param=system.switch_cpus[0:4].fuPool.FUList[1].count=64 \
  --param=system.switch_cpus[0:4].fuPool.FUList[2].count=64 \
  --param=system.switch_cpus[0:4].fuPool.FUList[3].count=64 \
  --param=system.switch_cpus[0:4].fuPool.FUList[4].count=64 \
...
CKPT_DIR=${GIT_ROOT}/ckpts/"250403-mac-"$num_nics"NIC"-$num_queues"Qs-SVE-16RXD-FF-B64-tbl-parallel-dmbfix"-$GUEST_SCRIPT
if [[ -z "$num_nics" ]]; then
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/250403-mac-SVE-16RXD-FF-B64-tbl-parallel-dmbfix-SingleQueue/$num_nics"NIC-"$num_queues"Qs-1core-ckp-"$GUEST_SCRIPT
  setup_dirs
  ...
  RUNDIR=${GIT_ROOT}/rundir/$(date +%Y%m%d)-mac-dpdk-set-1core-l3-2port-4ns-sve-16RXD-FF-B64-tbl-pDMA2-dmbfix-unlimited-spec-predreg-wait-long-log/$num_nics"NIC-"$num_queues"Qs-"$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$RATE"Gbps-ddio-enabled"-$GUEST_SCRIPT
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG  --cpu-clock=$Freq -r 3 --loadgen-start=11539382619990 --rel-max-tick=400010000000 --packet-rate=$PACKET_RATE --packet-size=$PACKET_SIZE --loadgen-mode=$LOADGENMODE \
  --warmup-dpdk 200000000000"

  # CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG  --cpu-clock=$Freq -r 3 --loadgen-start=11339402619990 --rel-max-tick=400010000000 --packet-rate=$PACKET_RATE --packet-size=$PACKET_SIZE --loadgen-mode=$LOADGENMODE \
