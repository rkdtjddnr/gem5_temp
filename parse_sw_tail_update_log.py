...
def parse_log_file(filename, batch_size=64, ring_size=1024):
    # 파일에서 로그 데이터 읽기
    df = pd.read_csv(filename, sep=",")
    
    # 컬럼 이름 넣기
    df.columns = ["Tick", "Tail"]
    ...
    for _, row in df.iterrows():
        if row["Tail"] % batch_size == 0:
            update_points.append((row["Tick"], row["Tail"]))
    ...
    # Find the row with time_intervals is larger than threshold
    threshold = 1400
    for i in range(0, len(time_intervals)):
        if time_intervals[i] > threshold:
            print(f"Time interval larger than {threshold} ns: time_intervals[{i}] = {time_intervals[i]}")
    
    # exclude the row with time_intervals larger than threshold
    time_intervals = [interval for interval in time_intervals if interval < threshold]
...
def main():
    ...
    print("Tail Update Time Intervals (ns):\n")
    #print time_intervals with \n for each element
    print('\n'.join(map(str, time_intervals)))
